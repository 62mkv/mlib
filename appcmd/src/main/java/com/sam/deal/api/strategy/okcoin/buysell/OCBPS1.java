...
public class OCBPS1 implements IBuyPointSelector {
	...
	public boolean isGoodBuyPoint(IStock s, ICashAccount ac) {
	    if (s instanceof OkCoinStock) {
	        ...
	        if (hs.isLstPriTurnaround(true) && hs.isLstPriUnderWaterLevel(0.1)) {
	            log.info("Stock trend turn up and at low 0.1 level, OCBPS1 return true.");
	            return true;
	        }
	        else {
                if (hs.getStockTrend() == eSTOCKTREND.SUP) {
                    log.info("Price trend is SUP, OCBPS1 return true!");
                    return true;
                }
	...
	public double getBuyQty(IStock s, ICashAccount ac) {
        ...
        if (s instanceof OkCoinStock && ac instanceof OkCoinCashAcnt) {
            OkCoinStock hs = (OkCoinStock)s;
            OkCoinCashAcnt hbac = (OkCoinCashAcnt)ac;
        ...
        log.info("IStock is not OkCoinStock(or OkCoinCashAcnt), return 0 as buyqty!");
		return 0;
	}
...
//                return true;
	    ...
	    log.info("IStock is not OkCoinStock(or OkCoinCashAcnt), can not buy huobi!");
	    return false;
	}
}
