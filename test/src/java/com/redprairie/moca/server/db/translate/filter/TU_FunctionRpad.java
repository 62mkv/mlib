 ...
 * @author mlange
 */
public class TU_FunctionRpad extends AbstractMocaTestCase {
    public void testTwoArguments() throws MocaException {   
        String sql;
        
        sql = "select '(' || rpad('foobar', 0) || ')' result from dual";
        runTest(sql, "result", "()");
        
        sql = "select '(' || rpad('foobar', 3) || ')' result from dual";
        runTest(sql, "result", "(foo)");
        
        sql = "select '(' || rpad('foobar', 6) || ')' result from dual";
        runTest(sql, "result", "(foobar)");
        
        sql = "select '(' || rpad('foobar', 10) || ')' result from dual";
        runTest(sql, "result", "(foobar    )");
    } 
