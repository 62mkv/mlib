...
package com.sam.moca.server.log.appender;
 ...
 */
@Plugin(name = "MocaRouting", category = "Core", elementType = "appender", printObject = true)
public final class MocaRoutingAppender extends AbstractAppender implements MocaAppender {
    ...
    private MocaRoutingAppender(final String name, final Filter filter, final boolean ignoreExceptions, final Routes routes,
    ...
    public void closeAppender(String appenderName) {
        AppenderControl appender = appenders.remove(appenderName);
        if (appender != null) {
            LOGGER.info("Stopping appender {}", appenderName);
            appender.getAppender().stop();
        }
    }
     ...
     */
    @PluginFactory
    public static MocaRoutingAppender createAppender(
        ...
        return new MocaRoutingAppender(name, filter, ignoreExceptions, routes, rewritePolicy, config);
    }
}
