...
/**
 * This custom infinispan command is used to stop tasks.
 * Supports stopping a list of tasks.
 * 
 ...
 * @author rrupp
 */
public class StopTasksCommand extends ClusterAdminCommand implements MultipleResponseNodeCommand {

    public final static byte COMMAND_ID = -125;
    
    public StopTasksCommand(List<String> taskIds) {
        this(taskIds, null);
    }
    
    public StopTasksCommand(List<String> taskIds, List<Address> requestAddresses) {
    ...
    public Map<String, NodeResponse> perform(InvocationContext ctx) {
        Map<String, NodeResponse> stoppedTasks = new HashMap<String, NodeResponse>(_taskIds.size());
        for (String taskId : _taskIds) {
            stoppedTasks.put(taskId, getAdmin().stopTaskOnNode(taskId));
        }
    ...
    protected Object onMocaVisitor(MocaVisitor visitor, InvocationContext ctx)
            throws Throwable {
        return visitor.visitStopTasksCommand(ctx, this);
    }
