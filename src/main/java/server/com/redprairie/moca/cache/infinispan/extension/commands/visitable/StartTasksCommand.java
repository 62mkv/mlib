...
/**
 * This custom infinispan command is used to start tasks.
 * Supports starting a list of tasks.
 * 
 ...
 * @author rrupp
 */
public class StartTasksCommand extends ClusterAdminCommand implements MultipleResponseNodeCommand {

    public final static byte COMMAND_ID = -123;

    public StartTasksCommand(List<String> taskIds) {
        this(taskIds, null);
    }
    
    public StartTasksCommand(List<String> taskIds, List<Address> requestAddresses) {
    ...
    public Map<String, NodeResponse> perform(InvocationContext ctx) {
        Map<String, NodeResponse> startedTasks = new HashMap<String, NodeResponse>(_taskIds.size());
        for (String taskId : _taskIds) {
            startedTasks.put(taskId, getAdmin().startTaskOnNode(taskId));
        }
    ...
    protected Object onMocaVisitor(MocaVisitor visitor, InvocationContext ctx)
            throws Throwable {
        return visitor.visitStartTasksCommand(ctx, this);
    }
