...
#include <mocaerr.h>
#include <mislib.h>
#define _toascii(a)      ((a)&0177)
#define EN      0
#define DE      1
#define CKS     2
#define MODS    3
typedef unsigned char BYTE;   /* BYTE = (VAX) ? int : char;   */
...
#else
        /* This has been added for use with mislib */
...
long misEncrypt(char *data, long length)
{
    ...
    /* We need a buffer a multiple of 16 bytes in size for the encryption */
    
    buffer_size = ((length - 1)/16 + 1) * 16;
    if((temp = calloc(buffer_size, 1)) == NULL)
    {
        return(eNO_MEMORY);
    }
    memset(temp, ' ', buffer_size);
    ...
    /* Because Sybase (where this will eventually end up gets confused by) */
    /* embedded ", and ' characters, we'll just substitute another value */
    /* if these are part of the encrypted string.                        */
    
    for(i = 0; i < length; i += 1)
    {
        if(*(data+i) == '"' || *(data+i) == '\'')
        {
            *(data+i) = 'x';    /* As good as anything */
        }
    }
