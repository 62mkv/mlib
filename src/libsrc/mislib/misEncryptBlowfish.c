...
/*
 * This section has been added for use with mislib
 */
struct mis__BlowfishKey
{
    BLOWFISH_CTX orig_ctx;
};

static void copyIn(unsigned char *data, unsigned long *left, unsigned long *right)
{
    *left = (data[0]<<24) | (data[1]<<16) | (data[2]<<8) | (data[3]);
    *right = (data[4]<<24) | (data[5]<<16) | (data[6]<<8) | (data[7]);
}

static void copyOut(unsigned char *data, unsigned long left, unsigned long right)
{
    data[3] = (unsigned char) (left & 0xff);
    data[2] = (unsigned char) ((left >> 8) & 0xff);
    data[1] = (unsigned char) ((left >> 16) & 0xff);
    data[0] = (unsigned char) ((left >> 24) & 0xff);
    data[7] = (unsigned char) (right & 0xff);
    data[6] = (unsigned char) ((right >> 8) & 0xff);
    data[5] = (unsigned char) ((right >> 16) & 0xff);
    data[4] = (unsigned char) ((right >> 24) & 0xff);
}
...
char *misEncryptBlowfish(char *data, long length, MIS_BLOWFISH_KEY *key, long *resultlen)
{
    ...
    memcpy(&ctx, &key->orig_ctx, sizeof(BLOWFISH_CTX));
    
    /* We need a buffer a multiple of 8 bytes in size for the encryption */
    buffer_size = ((length)/8 + 1) * 8;
    padbytes = buffer_size - length;
    ...
    memset(temp + length, padbytes, padbytes);

    for (i = 0; i < buffer_size; i += 8)
    {
        unsigned long left, right;
        copyIn(temp + i, &left, &right);
        Blowfish_Encrypt(&ctx, &left, &right);
        copyOut(temp + i, left, right);
    }

    *resultlen = buffer_size;
...
char *misDecryptBlowfish(char *data, long length, MIS_BLOWFISH_KEY *key, long *resultlen)
{
     ...
     */
    padbytes = (int)temp[buffer_size - 1];
    ...
    {
        *resultlen = buffer_size - padbytes;
    }
...
{
    MIS_BLOWFISH_KEY *out = malloc(sizeof(MIS_BLOWFISH_KEY));
    Blowfish_Init(&out->orig_ctx, key, keylen);
    return out;
}
