 ...
 *  $Author$
 *
 *  Description: This routine sets the string value of a named column.
 *
 *  $Copyright-Start$
 *
 *  Copyright (c) 2016
                             ...
                             char *value)
{
    ...
    if (sqlGetDataTypeByPos(res, colNumber) != COMTYP_STRING)
	return(eINVALID_OPERATION);

    /* Change the value of the column. */
    return(sql_AddRowItem(res, row, colNumber, strlen(value), (void *) value));
}
