...
{
    /// <summary>
    /// A date mapping delegate class
    /// </summary>
    public class DateMappingDelegate : MappingDelegate
    {
        /// <summary>
        ...
        public DateMappingDelegate(ColumnMap map)
            : base(map)
        {
        }
        ...
        public override void SetValue(object target, MocaType type, string data)
        {
            ...
            DateTime dateTime;
            if (EncodingBase.TryParseDateTime(data, out dateTime))
            {
                SetPropertyValue(target, dateTime);
            }
            else if (DateTime.TryParse(data, out dateTime))
            {
                SetPropertyValue(target, dateTime);
            }
            else
